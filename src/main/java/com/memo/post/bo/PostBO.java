...
public class PostBO {
	...
	public List<Post> getPostListByUserId(int userId, Integer prevId, Integer nextId) {
		...
		// 만약 4 3 2 페이지로 있을 때
		// 1) 다음: 2보다 작은 3개 DESC
		// 2) 이전: 4보다 큰 3개 ASC로 가져온다 => 5 6 7 => BO에서 reverse(뒤집는다.) 7 6 5
		// 3) 페이징 X: 최신순 3 DESC
	...
	public boolean isPrevLastPageByUserId(int userId, int prevId) {
		...
		return maxPostId == prevId; // 같으면 마지막 페이지
	}
	...
	public boolean isNextLastPageByUserId(int userId, int nextId) {
		...
		return minPostId == nextId; // 같으면 마지막 페이지
	}
	...
	// input: 파라미터들(userId, loginId, postId, subject, content, file)
	// output: X
	public void updatePostByPostId(
		...
		// 기존글 가져온다.(1. 이미지교체시 삭제하기 위해  2. 업데이트 대상이 있는지 확인)
		...
		if (file != null) {
			...
			if (imagePath != null && post.getImagePath() != null) {
				// 폴더와 이미지 제거(서버에서)
				fileManagerService.deleteFile(post.getImagePath()); // 기존 이미지 삭제
			}
		}
		// DB update
